...
export function ManagePlacesPage({ onBack, isAdmin }: ManagePlacesPageProps) {
  ...
  const saveEdit = async () => {
    ...
    try { await placesApi.update(edit.id, form); flash('Saved ✓'); setEdit(null); loadMine(); }
    catch (e: any) { flash(e?.message || 'Could not save'); } finally { setSaving(false); }
  };
  const onPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (!f || !edit) return;
    try { await placesApi.uploadPhoto(edit.id, f); flash('Photo uploaded ✓'); loadMine(); } catch { flash('Upload failed'); }
    e.target.value = '';
  };
  ...
  const statusChip = (s: string) => {
    ...
    return <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${map[s] || 'bg-slate-100 text-slate-500'}`}>{s}</span>;
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-display text-2xl font-bold text-[#2b2521]">Manage</h1>
      </div>
        ...
        <div className="space-y-3">
          {mine.length === 0 && (
            <div className="bg-white rounded-3xl p-8 text-center border border-slate-200">
              ...
              <p className="text-sm font-medium text-slate-700 mb-1">No businesses yet</p>
              <p className="text-xs text-slate-500">Add your business or claim an existing listing to manage it here.</p>
            </div>
          )}
          {mine.map((p) => (
            <div key={p.id} className="bg-white rounded-3xl p-4 border border-slate-200 shadow-sm flex items-center gap-3">
              ...
              <button onClick={() => openEdit(p)} className="px-3 py-2 rounded-xl bg-[#f1ebff] text-[#6200FF] text-sm font-semibold hover:bg-[#e3d6ff] shrink-0">Edit</button>
            </div>
          ))}
        </div>
      )}
      ...
      {!loading && tab === 'admin' && isAdmin && (
        <div className="space-y-6">
          <div>
            <h3 className="font-semibold text-slate-900 mb-3">Pending approvals ({pending.length})</h3>
            <div className="space-y-2">
              ...
              {pending.map((p) => (
                <div key={p.id} className="bg-white rounded-2xl p-3 border border-slate-200 flex items-center gap-3">
                  ...
                  <button onClick={() => adminApi.approvePlace(p.id).then(() => { flash('Approved'); loadAdmin(); })} className="w-9 h-9 rounded-lg bg-emerald-500 text-white flex items-center justify-center"><Check size={16} /></button>
                  <button onClick={() => adminApi.rejectPlace(p.id).then(() => { flash('Rejected'); loadAdmin(); })} className="w-9 h-9 rounded-lg bg-rose-500 text-white flex items-center justify-center"><X size={16} /></button>
                </div>
              ))}
            </div>
          </div>
          <div>
            <h3 className="font-semibold text-slate-900 mb-3">Reported reviews ({reports.length})</h3>
            <div className="space-y-2">
              ...
              {reports.map((r: any) => (
                <div key={r.id} className="bg-white rounded-2xl p-3 border border-slate-200 flex items-center gap-3">
                  ...
                  <button onClick={() => adminApi.hideReview(r.reviewId ?? r.review_id).then(() => flash('Hidden'))} className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-xs font-semibold flex items-center gap-1"><EyeOff size={13} /> Hide</button>
                  <button onClick={() => adminApi.deleteReview(r.reviewId ?? r.review_id).then(() => flash('Deleted'))} className="px-3 py-1.5 rounded-lg bg-rose-50 text-rose-600 text-xs font-semibold flex items-center gap-1"><Trash2 size={13} /> Delete</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setEdit(null)}
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()}
              ...
              <div className="p-6 space-y-3 max-h-[70vh] overflow-y-auto">
                <Field icon={<Phone size={16} />} label="Phone" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} placeholder="+994 ..." />
                <Field icon={<Globe size={16} />} label="Website" value={form.website} onChange={(v) => setForm({ ...form, website: v })} placeholder="https://" />
                <Field icon={<Clock size={16} />} label="Opening hours" value={form.openingHours} onChange={(v) => setForm({ ...form, openingHours: v })} placeholder="Mo-Su 09:00-23:00" />
                <Field icon={<MapPin size={16} />} label="Address / city" value={form.city} onChange={(v) => setForm({ ...form, city: v })} />
                <div>
                ...
                <button onClick={() => fileRef.current?.click()} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 hover:border-[#6200FF] hover:text-[#6200FF] text-sm font-semibold"><ImagePlus size={17} /> Upload a real photo</button>
                <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={onPhoto} />
              </div>
              <div className="flex gap-3 p-6 pt-2">
                <button onClick={() => setEdit(null)} className="flex-1 py-3 rounded-xl border border-slate-200 font-semibold text-slate-600 hover:bg-slate-50">Cancel</button>
                <button onClick={saveEdit} disabled={saving} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#6200FF] text-white font-semibold hover:bg-[#5400dd] disabled:opacity-60">{saving ? <Loader2 size={17} className="animate-spin" /> : <><Check size={17} /> Save</>}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
